	...
	)
{
	...
	case Be::LONG:
		ok = BlueExtractInt( v, value->mLong );
		break;

	case Be::BYTE:
		if ((ok = PyInt_Check(v)) == true)
	...
	case Be::FLOAT:
		ok = BlueExtractFloat( v, value->mFloat );
		break;

	case Be::FLOATARRAY:
		if( var->mIID == &BlueMatrixIID )
		{
			ok = BlueExtractMatrix( v, &value->mFloat, var->GetFloatArraySize() );
			if( !ok )
			{
		...
		{
			ok = BlueExtractVector( v, &value->mFloat, var->GetFloatArraySize() );
			if( !ok )
			{
	...
	case Be::BOOL:
		ok = BlueExtractBool( v, value->mBool );
		break;

	case Be::IROOT:
		obj = BlueUnwrapObjectFromPython(v);
		...
		{
			CCopier copier;
			ok = copier.CopyTo(obj, (IRoot**)&value);
		}
		break;
	...
	case Be::INT64:
		if ((ok = PyLong_Check(v)) == true) {
			int64_t t = PyLong_AsLongLong(v);
			if (t == -1 && PyErr_Occurred()) 
				return false;
			value->mInt64 = t;
		} 
		else if ((ok = PyInt_Check(v)) == true)
		{
			int64_t t = PyInt_AsLong(v);
			if (t == -1 && PyErr_Occurred()) 
				return false;
			value->mInt64 = t;
		}
	...
	case Be::SCRIPTCALLBACK:
		{
			...
			ok = BlueExtractArgumentImpl( v, s, 0, std::false_type() );
		}
		break;
	...
	case Be::SHAREDSTRING:
		{
			...
			std::string str;
			ok = BlueExtractString( v, str );
			s = BlueSharedString( str );
		}
		break;

	default:
		PyErr_SetString(PyExc_TypeError, "Error converting type from python to blue.");
		break;
	}
